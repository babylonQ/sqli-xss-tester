...
#	opener2.open('http://192.168.57.100/security.php?phpids=on')
	...
	for payload in sqlipayloads:
		count += 1
		resp = opener.open('http://192.168.1.210/vulnerabilities/sqli/?id='+urllib.quote_plus(payload)+sufix) 
...
#	break
	...
	count = 0
        for payload in xsspayloads:
               count += 1
               resp = opener.open('http://192.168.1.210/vulnerabilities/xss_r/?name='+urllib.quote_plus(payload)+sufix) 
               print "Payload " + str(count) + ": " + payload + " passed to the web app"
               response = resp.read()
               # Check if WAF has been triggered
               if "Hacking" in response:
